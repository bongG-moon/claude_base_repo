...
try {
    ...
    $onboardingGuide = Join-Path $releasePlugin 'resources\manuals\Company-Agent-Guide.html'
    if (Test-Path -LiteralPath $onboardingGuide -PathType Leaf) { Write-Host "처음이라면 이 파일을 열고 1단계부터 따라 해보세요 (선택): $onboardingGuide" }
    $markdownGuide = Join-Path $releasePlugin 'resources\manuals\README.md'
    if (Test-Path -LiteralPath $markdownGuide -PathType Leaf) { Write-Host "글 안내서 (MD): $markdownGuide" }
