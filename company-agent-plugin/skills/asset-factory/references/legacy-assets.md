...
- Script Tool의 JSON 검증 버전이 없는 과거 영수증도 재시험 대상입니다. 실행 전 입력, 실행 후 출력을 현재 조건으로 확인한 새 영수증을 사용합니다. 지원 스키마 조건은 `platform-tools.md`와 같으며 지원하지 않는 조건을 삭제해 시험만 통과시키지 않습니다.
