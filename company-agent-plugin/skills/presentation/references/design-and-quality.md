...
대표 디자인은 `design-review.md`의 전체 HTML 초안으로 먼저 확인한다.
원하는 디자인이 있으면 표지·본문·표/차트 캡처 2~3장을 제안한다. 1장 또는 기존
PPTX/저장한 HTML도 허용하며 이미 디자인을 정했으면 다시 요구하지 않는다.
캡처는 직접 보고 색상·글꼴·내용 구성 원칙을 적용한다. 캡처를 통째로 PPT에 붙이지 않는다.
...
지정 PPTX 양식은 ppt-inspect 후 허용된 미리보기 경로로 실제 화면을 확인한다.
...
조합에서 결정된다. 자유 배치는 `native-layout.md`의 elements 좌표를 사용한다. 지표는 fact ID만 받으며 수치를 따로
...
5. 같은 workFile 내부에서 수정하고 다시 확인한다. 최종 전달은 한 번이며 이전 원본은 보존한다.
