...
설치된 공통 `business office-read --spec ... --session "현재 세션 ID"` 명령은 대화 승인을 확인한 뒤 PPT/Word 요청을
`Read-CompanyOffice.py` → `company_agent/office_pywin32.py`로 전달한다.
사용자에게 Python/JSON 작성을 요구하지 않는다. 누락된 경로·범위와 읽기 승인만 질문한다.
...
같은 작업을 반복하지 않는다. 최종 `diagnostics.progress.stageMs`에는 준비·대화 승인
확인·열기·추출·닫기 등의 시간이 남는다. 사용자 응답 대기는 포함하지 않는다. 원문은 기록하지 않는다.

- 실행 진입점의 Python 확인은 후보별 10초 제한이며 중복 경로는 검사하지 않는다.
  시간 초과 시 다른 Python으로 재시도하지 않고 `office_bootstrap_timeout`으로 중단한다.
- 별도 확인 창은 열지 않는다. `input_required`의 질문을 Claude에서 보여주고 승인을
  기다린다. 동일 세션·폴더·파일 상태·범위의 승인은 15분 이내 한 번만 유효하다.
  승인 없이는 Office를 열지 않고 대기 명령도 실행하지 않는다.
...
내용 전달 전 Claude 대화에서 확인받는다. 실제 접근 거절 또는 명시된 제한은 중단한다.
