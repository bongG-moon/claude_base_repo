...
`business office-read --spec ...`는 확장자가 xlsx/csv이면 항상 설치본의
`Read-CompanyExcel.py` → `company_agent/excel_xlwings.py`를 호출한다.
일반 파일도 같은 고정 경로를 쓴다. DRM을 제거하거나 복호화된 사본을 저장하지 않는다.
라이브러리 부족, 파일 열기 실패, 읽기 거절 뒤 다른 엔진으로 재시도하지 않는다.
...
- 읽은 내용을 AI 대화에 전달하는 기존 확인 창은 유지한다.
...
- 내용 읽기/추출/AI 사용이 명시적으로 금지된 문서를 이 절차로 다시 추출하지 않는다.
