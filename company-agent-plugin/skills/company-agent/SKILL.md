...
prefix in a Bash tool; it invokes the bundled runtime even without Python on PATH.
Use the indicated `stateRoot`, never assume the global state path. Read the full
relevant personal Skill from the hook's `personalSkills` list before applying it.
For more personal Skills run `skill search "<task>"` with the same prefix. These
files are active by contextual retrieval and need not appear in the slash menu.
...
2. Keep the current conversation as the user-facing coordinator. Do not expose internal delegation mechanics unless the user asks.
   For code, Script, MCP, or executable Skill work, read `../karpathy-guidelines/SKILL.md` and pass its concise four principles to the worker. Do not preload the upstream reference or apply a second planning/interview loop. Ordinary business writing does not need this coding Skill.
