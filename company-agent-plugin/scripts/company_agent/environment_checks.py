...
def inspect_environment():
    packages = {name: _module(name) for name in ('xlwings', 'pandas', 'win32com', 'pptx')}
    ...
    return {'readOnly': True, 'python': sys.executable, 'packages': packages, 'officeRegistration': office,
            'features': [
                ...
                {'name': 'Excel·CSV 읽기', 'status': ready(office['excel'] and packages['xlwings'] and packages['pandas']), 'requires': 'Excel + xlwings + pandas'},
                {'name': 'PPT 읽기', 'status': ready(office['powerpoint'] and packages['win32com']), 'requires': 'PowerPoint + pywin32'},
                {'name': 'Word 읽기', 'status': ready(office['word'] and packages['win32com']), 'requires': 'Word + pywin32'},
                ...
                {'name': 'PDF·이미지 생성', 'status': '별도 지원 도구 확인 필요', 'requires': '기본 Office 읽기 기능에 포함되지 않음'}],
