...
- For office-reader, use the supplied officeReadCommand unchanged; otherwise follow its cliCommand fallback, omitting only unavailable session/state options for hook binding. Preserve explicit parent context. Missing metadata is not an Office/DRM denial or a reason to substitute a parser. Leave unavailable markers/learning to the coordinator; return actual results without invented IDs.
