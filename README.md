...
처음이라면 **[Company Agent 사용자 안내서](docs/Company-Agent-사용자-안내서.html)**를 여세요. 5부 43장과 글꼴까지 담은 단독 HTML입니다. Markdown이 편하면 **[안내서 목차](docs/README.md)**에서 시작하기, Claude Code 기본 사용법, 업무 예문, 하네스 설명, 명령어·단축키를 골라 읽을 수 있습니다. 회사 자료나 참고 PPT 없이 가상 자료로 연습합니다.

개발용 **[Company Workspace 로컬 업무 화면](docs/LOCAL_WORKSPACE.md)**을 추가했습니다. `Company-Workspace.vbs`를 더블클릭하면 기존 Claude Code 설정을 사용하는 채팅·파일 선택·질문·승인 화면이 열립니다. 현재 [1.4.25 Release](https://github.com/bongG-moon/claude_base_repo/releases/tag/v1.4.25)에 공개된 별도 Workspace 묶음은 **0.8**(`company-workspace-preview-0.8-*.zip`)입니다. 하네스 설치 ZIP과 별개이며 사내 HCP·DRM 문서의 실제 업무 검증은 필요합니다.

Workspace는 **회사 공통 / 개인 전체 / 이 프로젝트**를 먼저 나누고 각 범위에서 **기억·지식 / 업무 구성**을 확인합니다. 새 기억·지식·스킬·도구는 개인 전체 또는 이 프로젝트를 선택한 뒤 저장하며 회사 공통은 읽기 전용입니다. 자동 학습은 기존 설치 범위에서만 작동하고 명시적 저장 선택으로 범위가 바뀌지 않습니다. 5단계 첫 업무 코스와 사용량/대기 시간·결과 확인도 제공합니다. 화면 조회에 추가 모델 호출은 없고 기존 데이터·로그인·모델·MCP 설정은 보존합니다. 공개 배포 조합은 **하네스 1.4.25와 Workspace 0.8**이며 기존 앱은 먼저 종료하세요. 1.4.23에서 제거한 Office 읽기의 전용 결과 연결과 가상 보고서 체험을 유지하며, 1.4.25에 첨부한 Workspace ZIP은 안내서를 갱신한 묶음입니다. 기존 설치가 자동 갱신되지는 않습니다.

현재 소스의 **Workspace 0.9는 로컬 시험 버전**입니다. 시작할 때 로그인한 본인의 SID·세션·프로필을 확인하고, 관리자 권한으로 시작된 경우 같은 사용자의 검증된 일반 권한 UAC 연결 토큰이 있을 때만 한 번 다시 실행합니다. 일반 권한 토큰이 없거나 회사 정책으로 확인·실행이 차단되면 한국어로 확인된 이유를 안내하고 중단합니다. 관리자 권한으로 AI를 실행하거나 UAC·인증·기존 설정을 변경하지 않습니다. 코어 버전은 1.4.25로 유지하며, `company-workspace-preview-0.9-*.zip`을 로컬에서 만들어도 GitHub에 게시되지 않습니다. 별도 게시 전까지 기존 Release의 Workspace 0.8은 그대로입니다.

[Workspace 0.3 구현·검증 결과](docs/VALIDATION_BEGINNER_WORKSPACE_2026-09-19.md)에서 확인된 동작과 사내 환경에서 더 확인할 항목을 구분했습니다.

**Workspace가 `WS-33`으로 시작되지 않나요?** 이 저장소를 압축 해제한 폴더에서 `Check-Workspace.cmd`를 일반 더블클릭하세요. 같은 폴더에 생성되는 `Workspace-Diagnostic-….json`으로 권한 상태를 확인할 수 있습니다. 설정·권한·로그인은 변경하지 않으며 결과를 자동 전송하지 않습니다. [진단 파일 사용법](docs/WORKSPACE_STARTUP_DIAGNOSTIC.md)을 참고하세요.
...
통합 가이드 전체를 매 요청의 문맥에 넣지 않습니다. 문서 열기와 실습 요청 전송은 별개입니다. 사용자 안내서와 통합 가이드는 같은 전체 본문을 담고, 나머지 기존 설명용 HTML 3개는 짧은 호환 링크입니다. 담당자용 검증·설치 문서는 별도 소스로 보존합니다.
