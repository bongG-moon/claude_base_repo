...
const elements={};
for(const id of ['length-choice','mode-choice','choice-result','copy-status','selected-design','copy-choice'])elements[id]=new Element();
...
const context=vm.createContext({document:{querySelectorAll:()=>buttons,getElementById:id=>elements[id]},navigator});
vm.runInContext(script,context);
assert.equal(buttons.length,10);
assert.match(html,/<details id="additional-designs" class="design-options">/);
assert.equal((html.match(/class="design-card"/g)||[]).length,8);
...
for(const choice of [...new Set(buttons.map(b=>b.dataset.choice))]){
  ...
  assert.match(elements['choice-result'].value,new RegExp(button.dataset.label.replace(/[()]/g,'\\$&')));
