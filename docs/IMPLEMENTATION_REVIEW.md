...
구현의 기준은 이미 사내 모델과 연결된 Windows Claude Code입니다. 모델 연결·API 변환 계층을 다시 구축하지 않습니다. 현재 배포 버전은 1.1.1이며 1.1.0의 자동 학습을 유지하고 기존 Company Agent의 버전 비교·업데이트 안내를 추가했습니다. 일반 Update는 기존 사용자 규칙·Hook과 개인 상태를 보존하며, 별도로 명시한 Replace만 규칙·Hook 비활성화를 수행합니다. 아래는 코드에서 제공하는 기능과 별도 업무 검증이 필요한 범위를 구분한 점검입니다.
