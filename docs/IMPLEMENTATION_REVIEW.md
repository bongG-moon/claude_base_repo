...
구현의 기준은 이미 사내 모델과 연결된 Windows Claude Code입니다. 모델 연결·API 변환 계층을 다시 구축하지 않습니다. 현재 배포 버전은 1.1.2이며 자동 학습과 기존 Company Agent의 업데이트 안내를 유지합니다. 일반 더블클릭도 높은 권한으로 감지될 수 있는 회사 PC를 위해 관리자 여부 일괄 차단을 본인 사용자·세션·프로필 확인으로 바꾸고, Claude 실행 파일과 설정 위치를 각각 확인합니다. 일반 Update는 기존 사용자 규칙·Hook과 개인 상태를 보존하며, 별도로 명시한 Replace만 규칙·Hook 비활성화를 수행합니다. 아래는 코드에서 제공하는 기능과 별도 업무 검증이 필요한 범위를 구분한 점검입니다.
