# 업데이트와 개인 상태 보존 — 1.4.12

현재 1.4.12도 공통 엔진과 개인 자료를 분리하는 업데이트 원칙을 유지한다. 기존 상태 형식이 지원될 때만 같은 범위의 새 버전으로 업데이트하며, 지원하지 않는 형식은 보존 후 중단한다. 아래는 일반 Company Agent 업데이트와 별도로 요청한 기존 개인 하네스 교체를 구분한 안내다. 이전 `0.3.x`는 배포 전 개발 버전이다.
