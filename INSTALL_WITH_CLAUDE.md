...
현재 1.1.1 기본 ZIP에는 Python 실행 파일이나 DLL을 포함하지 않는다. 기존 사내 모델 설정을 그대로 사용한다. 1.0.0이나 1.1.0을 설치한 PC도 동일 범위로 업데이트할 수 있으며 개인 자료와 자동 학습 설정은 유지한다. 이전 `0.3.x`는 배포 전 개발 버전이다.
